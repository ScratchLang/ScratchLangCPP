...
void startloop(const char *a1 = "", int argus = 0, char *arguv[] = {}) {
    ...
    try {
        ...
        try {
            filesystem::current_path("data");
            realcwd = filesystem::current_path().parent_path().string();
        } catch (filesystem::filesystem_error) {
            error("Failed to find the 'data' directory!");
            exit(0);
        }
    }
...
#ifdef _WIN32
        bool connected = InternetCheckConnectionA("https://github.com",
                                                  FLAG_ICC_FORCE_CONNECTION, 0);
        if (connected) {
            std::ofstream outputFile("version", std::ios::binary);
            try {
            ...
            cout << RED + "Failed to connect to the internet." + NC << endl;
            sleep(3000);
        }
#else
        system("wget -q "
    ...
    if (argus > 1) {
        if (strcmp(arguv[1], "--help") == 0) {
            ...
            cout << "Usage: scratchlang.exe [OPTIONS]\n"
                 ...
                 << endl;
#else
            cout << "Usage: scratchlang [OPTIONS]\n"
                 << endl
                 << "  -1                Create a project." << endl
                 << "  -2                Remove a project." << endl
                 << "  -3                Compile a project." << endl
                 ...
                 << endl
                 << "Examples:" << endl
                 << "  ./scratchlang -4 ~/project.sb3" << endl;
#endif
            exit(0);
        }
        args = true;
...
void inputloop(string ia1, char *aac[] = {}, bool aab = false,
    ...
    if (inp == "1") {
        ...
        string ssi;
#ifdef _WIN32
        filedialog("Choose a ScratchScript Archive", "../exports", 1,
                   filterPatterns, ssi);
#else
        ssi =
